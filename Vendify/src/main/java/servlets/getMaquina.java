...
public class getMaquina extends HttpServlet{
     ...
     protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
                ...
                System.out.println("-- Get Machines information from DB--");
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out = response.getWriter();
		try 
		{
			ArrayList<Maquina> values =Logic.getMaquinasFromDB();
			String jsonMaquinas = new Gson().toJson(values);
			System.out.println("JSON Values=> "+jsonMaquinas);
