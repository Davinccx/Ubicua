...
document.addEventListener('DOMContentLoaded', (event) => {
    document.getElementById('boton-generar-historico').addEventListener('click', function () {
        fetch('http://localhost:8080/generateHistorico', {
            method: 'POST'
        })
                .then(response => response.json())
                .then(data => {
                    if (data.error) {
                        ...
                        document.getElementById('matricula2').value = data.matricula;
                        document.getElementById('parking_id2').value = data.parking_id;
                        document.getElementById('hora_inicio2').value = data.hora_inicio;
                        document.getElementById('hora_fin2').value = data.hora_fin;
                        document.getElementById('importe').value = data.importe+" €";
                       
                        alert('El historico se ha generado correctamente');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
